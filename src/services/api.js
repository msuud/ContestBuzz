...
const api = axios.create({
  baseURL: process.env.REACT_APP_API_URL || process.env.REACT_APP_CLIST_API_URL,
  timeout: 20000,
  headers: {
    "Content-Type": "application/json",
    Authorization: process.env.REACT_APP_CLIST_AUTHORIZATION,
  },
});
  ...
  async getContests() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      contests.push({
        ...
        platform: platforms[Math.floor(Math.random() * platforms.length)],
        url: process.env.REACT_APP_CLIST_API_URL,
        start_time: startTime.toISOString(),
        end_time: endTime.toISOString(),
        duration: (endTime - startTime) / (1000 * 60 * 60),
      });
